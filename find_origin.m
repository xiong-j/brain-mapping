...
function [new_img, origin] = find_origin(old_img, old_M, new_elem_size, verbose, bg, method)
...
%--------------------------------------------------------------------
function [img_slice,origin_slice] = trilinear(img, dim1, dim2, M, bg)
   ...
   for y = 1:ydim1
      ...
      %
      Y2X = Y2X + M(1,2);		% new_Y to old_X M is mapping 4x4 matrix
      ...
      for x = 1:xdim1
		...
		old_Z > 1-TINY & old_Z < zdim2+TINY	)
            ...
            img_slice(x,y) = v000*(1-dx)*(1-dy)*(1-dz) + ...
               ...
               v101*dx*(1-dy)*dz + ...
               v111*dx*dy*dz;
	    origin_slice(x,y,:)=[old_X,old_Y,old_Z]';
